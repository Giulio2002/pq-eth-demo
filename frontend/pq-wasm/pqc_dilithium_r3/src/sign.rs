...
pub fn crypto_sign_keypair(
  ...
  let mut tr = [0u8; CRHBYTES]; // FIPS 204: tr is 64 bytes
  ...
  // Compute H(rho, t1) — FIPS 204: tr = SHAKE256(pk)[0..64]
...
pub fn crypto_sign_signature(sig: &mut [u8], m: &[u8], sk: &[u8]) {
  ...
  let mut tr = [0u8; CRHBYTES]; // FIPS 204: 64-byte tr
  ...
  // Compute CRH(tr, msg) — FIPS 204: use full 64-byte tr
...
pub fn crypto_sign_verify(
  ...
  // Compute CRH(CRH(rho, t1), msg) — FIPS 204: 64-byte tr
